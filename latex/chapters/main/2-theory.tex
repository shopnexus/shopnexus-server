...
\textbf{Trong ShopNexus:} Hệ thống có 8 module: \textit{account}, \textit{catalog}, \textit{order}, \textit{inventory}, \textit{promotion}, \textit{analytic}, \textit{chat}, \textit{common}. Mỗi module có cấu trúc vertical-slice đồng nhất gồm 4 lớp: \texttt{biz/} chứa logic nghiệp vụ, \texttt{db/} chứa truy vấn SQLC, \texttt{model/} chứa kiểu dữ liệu, \texttt{transport/echo/} chứa HTTP handlers. Schema isolation được thực thi tại tầng database: mỗi module tạo schema riêng trong PostgreSQL (ví dụ \texttt{CREATE SCHEMA IF NOT EXISTS "order"} tại \texttt{internal/module/order/db/migrations/0001\_init.up.sql}) và không có foreign key xuyên schema -- đây là điều kiện để tách module thành microservice độc lập khi cần. Giao tiếp giữa module thực hiện qua interface \texttt{XxxBiz} được inject bằng Uber fx; khi chạy trong Restate handler, interface này được resolve thành Restate proxy client (sinh tự động bởi \texttt{cmd/genrestate}), đảm bảo mọi cross-module call đều có retry và deduplication.
