...
% Swimlanes
\draw[dashed, gray!50] (4.2,0.3) -- (4.2,-21);
\draw[dashed, gray!50] (9,0.3) -- (9,-21);
...
\node[act, below=of cart] (checkout) {Checkout\\(địa chỉ + vận chuyển)};
\node[act, below=of checkout, xshift=4.5cm] (reserve) {Reserve kho + Báo giá\\vận chuyển + Tính tổng};
\node[act, below=of reserve, xshift=-4.5cm] (pay) {Thanh toán\\(Ví + VNPay/COD)};
\node[act, below=of pay, xshift=4.5cm] (createitems) {Tạo paid items\\+ Lên lịch timeout};

% Phase 2: Payment callback
\node[act, below=of createitems] (callback) {Webhook xác nhận TT\\$\to$ timeout 48h seller};

% Phase 3: Seller confirm
\node[act, below=of callback, xshift=4.5cm] (inbox) {Xem mục đã TT};
\node[dec, below=0.6cm of inbox] (d1) {Xác nhận?};
\node[act, right=1cm of d1] (reject) {Từ chối\\(hoàn ví + giải phóng kho)};
\node[act, below=0.6cm of d1, xshift=-4.5cm] (create) {Tạo Order + Transport\\(platform tạo vận đơn)};
...
\node[act, below left=0.5cm and 1cm of d3] (autorefund) {Hoàn tiền vào ví};
\node[act, below right=0.5cm and 0.5cm of d3] (dispute) {Dispute (Admin)};
\node[finish] (e) at (6.5,-21) {};

% Arrows — Phase 1: Checkout + Payment
\draw[arr] (s) -- (cart);
\draw[arr] (cart) -- (checkout);
\draw[arr] (checkout) -- (reserve);
\draw[arr] (reserve) -- (pay);
\draw[arr] (pay) -- (createitems);
\draw[arr] (createitems) -- (callback);

% Arrows — Phase 3: Seller
\draw[arr] (callback) -- (inbox);
\draw[arr] (inbox) -- (d1);
...
\draw[arr] (dispute) |- (e);
\end{tikzpicture}
\caption{Quy trình đặt hàng, thanh toán, và hoàn trả trong ShopNexus (pay-first)}
