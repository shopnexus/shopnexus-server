...
Hệ thống ShopNexus được tổ chức theo mô hình \textbf{durable microservices}: toàn bộ mã nguồn trong một repository, nhưng mỗi module đã sẵn sàng triển khai độc lập như microservice --- database riêng (schema isolation), execution riêng (Restate service handler), giao tiếp qua HTTP proxy (cùng protocol bất kể co-located hay distributed). Phần mềm được xây dựng bằng Go~1.26, sử dụng framework \textbf{Restate} cho thực thi bền vững (durable execution) và \textbf{Uber~fx} cho dependency injection. Kiến trúc được phân rã theo ba kỹ thuật: \textit{phân tầng} (layering), \textit{phân vùng} (segmentation), và \textit{trích lớp cơ bản} (factoring).
...
\begin{longtable}{@{}llL{8.5cm}@{}}
\caption{Danh sách module nghiệp vụ}\label{tab:modules}\\
\toprule
\rowcolor{headerblue}
...
% Biz layer
\node[layer, minimum width=15cm, minimum height=2.7cm, label={[lbl]left:Biz}] (biz) at (7,3) {};
...
% External services
\node[ext] (payment) at (2,-0.8)   {Payment};
\node[ext] (transport) at (5,-0.8)    {Transport};
\node[ext] (geocoding) at (8,-0.8){Geocoding};
\node[ext] (llm) at (11,-0.8) {LLM};
...
\draw[arr] (biz) -- (data);
\draw[arr, dashed] (order) -- (payment);
\draw[arr, dashed] (order) -- (transport);
\draw[arr, dashed] (catalog) -- (llm);
\draw[arr, dashed] (common) -- (geocoding);
