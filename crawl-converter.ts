    ...
    try {
      ...
      const spu = await prisma.productSpu.create({
        data: {
          ...
          specifications:
            Array.isArray(input["Product Specifications"])
              ? input["Product Specifications"]
              : [],
          ...
          products: {
            create: {
              ...
              package_details: {
                weight_grams: faker.number.int({ min: 100, max: 2000 }),
                length_cm: faker.number.int({ min: 5, max: 50 }),
                width_cm: faker.number.int({ min: 5, max: 50 }),
                height_cm: faker.number.int({ min: 5, max: 50 }),
              },
            },
          },
        },
