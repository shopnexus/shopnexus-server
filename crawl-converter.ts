...
function toBigInt(value: any, fallback: bigint = BigInt(0)): bigint {
  ...
  try { return BigInt(Math.trunc(n)) } catch { return fallback }
}
...
function mapAttributes(input: any): Array<{ name: string; value: string }> {
  ...
  if (variations.length && chosen.length && typeof chosen[0]?.value === 'string') {
    ...
    return variations.map((v: any, idx: number) => ({ name: v.name || `Option ${idx + 1}`, value: chosenValues[idx] || (Array.isArray(v.variations) ? v.variations[0] : '') }))
  }
  // Fallback: take first option of each variation
  if (variations.length) {
    return variations.map((v: any, idx: number) => ({ name: v.name || `Option ${idx + 1}`, value: Array.isArray(v.variations) && v.variations.length ? String(v.variations[0]) : '' }))
  }
  return []
}
...
async function main(inputs: InputProduct[] = []) {
...
const inputs: InputProduct[] = JSON.parse(fs.readFileSync('data.json', 'utf-8'))
